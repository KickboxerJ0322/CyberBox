...
export const api = {
  startLab: () => request<LabSession>('/api/labs', { method: 'POST' }),
  stopLab: (id: string) => request<{ ok: boolean }>(`/api/labs/${id}`, { method: 'DELETE' }),
  targetStatus: (id: string) => request<TargetStatus>(`/api/labs/${id}/target-status`, { cache: 'no-store' }),
  aiStatus: () => request<AiStatus>('/api/ai/status', { cache: 'no-store' }),
  explain: (id: string, lessonId: number, command: string, output: string) =>
